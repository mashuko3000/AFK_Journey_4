...
void bigfloat::parse_number(const char *str, int &index, big_int &result) {
    if (str == nullptr) {
...
bigfloat bigfloat::abs() const {
    return bigfloat(boost::multiprecision::abs(numerator_), boost::multiprecision::abs(denominator_));
}

bigfloat bigfloat::operator-() const {
    return bigfloat(numerator_, -denominator_);
}

big_int bigfloat::factorial(const big_int &n) {
    if (n < 0) {
...
big_int bigfloat::factorial(const unsigned long &n) {
    if (n < 0) {
        throw std::invalid_argument("Factorial is not defined for negative numbers");
    }
    if (n == 0 || n == 1) {
...
bigfloat bigfloat::pow(unsigned int exp) const {
    if (exp == 0) {
        if (numerator_ == 0) {
...
bigfloat bigfloat::reciprocal() const {
    ...
    bigfloat result(denominator_, numerator_);
    result.simplify();
    return result;
}

bigfloat bigfloat::sqrt(const bigfloat &eps) const {
    if (*this < bigfloat(0)) {
    ...
    do {
        ...
        bigfloat diff = (guess - prev_guess).abs();
        bigfloat rel_diff = diff / guess.abs();
...
*/
/*
big_int bigfloat::power_base(const big_int& base, const big_int& exp) {
