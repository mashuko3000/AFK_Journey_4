...
std::string bigfloat::to_decimal(int precision) const {
...
std::ostream &operator<<(std::ostream &out, const bigfloat &num) noexcept {
    ...
    out << num.numerator_ << "/" << boost::multiprecision::abs(num.denominator_);
    return out;
    /*out << num.to_decimal(10);
    return out;*/
}
