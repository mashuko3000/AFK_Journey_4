...
class poisson_distribution : public i_distribution_model
{
private:
    double m_lambda;

public:
    explicit poisson_distribution(const double lambda) : m_lambda(lambda)
    {
    ...
    size_t get_next_index() override
    {
        ...
        double p = std::exp(-m_lambda);
        double sum = p;
        size_t k = 0;
        while (r > sum)
        {
            k++;
            p *= m_lambda / static_cast<double>(k);
            sum += p;
        }
    ...
    i_distribution_model* clone() const override
    {
        return new poisson_distribution(*this);
    }
};
